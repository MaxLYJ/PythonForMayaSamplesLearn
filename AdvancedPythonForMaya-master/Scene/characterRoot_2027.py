...
def initializePlugin(plugin):
    # Add the directory that contains this plugin to the script path so Maya can find the
    # attribute editor template (AEcharacterRootTemplate.mel) that ships next to it.
    # Maya 2027 fix: we derive the path from this file's location instead of a hardcoded drive,
    # and os.getenv may return None in Python 3, so we guard against that.
    dirName = os.path.dirname(inspect.getfile(initializePlugin))
