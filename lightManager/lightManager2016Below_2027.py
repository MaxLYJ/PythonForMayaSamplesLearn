...
Maya 2027 version: ships with PySide6 / Shiboken6 (Qt6) and Python 3.
Note: Maya 2016-and-below dockControl is retained here only for teaching comparison; in real Maya 2027
      use the workspaceControl-based lightManager_2027.py instead.
"""
...
# Maya 2027 ships with PySide6 directly (Qt6), so we no longer need the Qt.py compatibility shim.
# Maya 2017-2023 used PySide2 (Qt5):     from PySide2 import QtWidgets, QtCore, QtGui
# Maya 2016 and below used PySide (Qt4):  from PySide import QtGui, QtCore  (QtWidgets was called QtGui)
from PySide6 import QtWidgets, QtCore, QtGui
# Maya 2027 uses Shiboken6 (bundled with PySide6) for wrapping C++ pointers into Python objects.
# Maya 2017-2023 used shiboken2; Maya 2016 and earlier used shiboken.
from shiboken6 import wrapInstance
# PySide6 exposes Signal directly from QtCore (as did PySide2). PyQt used pyqtSignal instead.
...
class LightWidget(QtWidgets.QWidget):
    ...
    def __init__(self, light):
        ...
        # Maya 2027 uses Python 3, where basestring no longer exists, so we check against str instead
...
def getMayaMainWindow():
    ...
    # Maya 2027 uses Python 3, where there is no long type, so we cast the pointer with int() instead of long()
